...
- **Default to single-agent.** Use parallel sub-agents for parallelizable work with 2+ independent file groups, sub-issues or independent tasks. Below that threshold, delegate bulk I/O work to sub-agent(s) to prevent main-thread context overrun (see `${CLAUDE_PLUGIN_ROOT}/_shared/composition.md` § "Main-thread overrun").
