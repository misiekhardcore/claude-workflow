...
**`${CLAUDE_PLUGIN_DATA}`** — a persistent directory (`~/.claude/plugins/data/claude-workflow/`) for cached state that survives plugin updates. Use it for installed dependencies, compiled indexes, or cached fetched data. See `_templates/AUTHORING.md` for the diff-then-install pattern when detecting dependency updates.
