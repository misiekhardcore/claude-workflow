...
Spawn `Agent("architecture/agents/constraint-analyzer.md")` — pass `problem`, `cwd`, `codebase_findings` and `patterns_findings` from step 1 outputs.
...
Scan for vague language or thin sections → spawn `Agent("architecture/agents/deepening-agent.md")` per gap — pass `gap`, `cwd`, and `context` (summary of prior research). Max 2 rounds; user approves before each dispatch.
