...
3. **TDD loop** per AC in work unit:
   a. Write failing test.
   b. Implement minimum code to pass.
   c. Refactor while tests stay green.
   d. Commit: `git commit -m "feat(<work-unit-id>): <description>"`.
4. **Context hygiene**: delegate bulk reads to sub-agent on concept shifts.
5. **Verify**: run type-check, lint, and unit tests scoped to touched files.
6. **Report**: emit output block (see § Output).
