...
- **`/discovery` → `/discover` rename announced** in `_templates/AUTHORING.md` but not yet executed — skill still lives at `skills/discovery/`. `/discovery` remains the correct command for now.
